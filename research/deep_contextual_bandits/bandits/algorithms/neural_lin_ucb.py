...
class NeuralLinUCB(BanditAlgorithm):
  ...
  def __init__(self, name, hparams, optimizer='RMS'):
    ...
    self.inv_a = np.concatenate(tuple([np.eye(self.n_d)[np.newaxis,:,:] for i in range(self.n_a)]), axis=0) / self.lam
  ...
  def update(self, context, action, reward):
    ...
    if self.t % self.update_freq_nn == 0:
        ...
        rewards = self.data_h.rewards[np.arange(actions.shape[0]),actions] # strange but data_h.rewards is of shape (n_samples, n_actions) so we select actions pulled by model

        self.a = np.dot(contexts.T, contexts) + np.concatenate(tuple([np.eye(self.n_d)[np.newaxis,:,:] for i in range(self.n_a)]), axis=0) * self.lam
        self.b = np.concatenate(tuple([np.dot(rewards[actions==action], contexts[actions==action])[np.newaxis,:] for action in range(self.n_a)]), axis=0)
        self.inv_a = np.concatenate(tuple([np.linalg.inv(self.a[action])[np.newaxis,:,:] for action in range(self.n_a)]), axis=0)
        self.theta = np.concatenate(tuple([np.dot(self.inv_a[action], self.b[action])[np.newaxis,:] for action in range(self.n_a)]), axis=0)
